...
On a local machine, I force-enabled `NaniteAtomicsSupported()` and the build system subsequently acted very strangely. XCBBuildService would crash in the middle of a build. That made the UnrealBuildTool execute in the background, with no way to track its progress in Xcode.
