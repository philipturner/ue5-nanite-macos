...
The fragment shader was transpiled from an HLSL shader, located at the path below. It was compiled with `RESOURCE_TYPE` set to either 0 or 5, making `ClearResource` a `RWBuffer`. I'll try chasing where it was initialized a setting `RESOURCE_TYPE` to 1. That should change the clear resource to `RWTexture2D`.
