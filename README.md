...
| - | ----------------------------------- | ---------------------------------- |
| `NaniteAtomicsSupported()` left as-is | Default build config; runs smoothly with Nanite disabled. <ins>Build time: unknown</ins> | Currently compiling, waiting on results, expecting a crash on launch. <ins>Build time: current ETA is 81 minutes (Xcode 13, from scratch)</ins> |
...
| `NaniteAtomicsSupported()` always returns true; its original code is commented out | This will be my final test | I plan to test this next. <ins>Build time: n/a minutes (Xcode 13, with previous compilation's build products present)</ins> |
