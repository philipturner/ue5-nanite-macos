...
To launch the Unreal Editor inside Xcode (where you can debug it when it crashes), repeat the process for creating `UnrealProject1`. Go through <b>GAMES > First Person > Project Defaults > BLUEPRINT</b> instead of <b>C++</b> and name it `UnrealProject2`. `UnrealProject1` was difficult to work with because I created it with Xcode 14 beta. Switching to Xcode 13 caused a popup that requested to recompile missing modules. Blueprint projects are easier to work with, so I recommend using them from now on.
