...
To launch the Unreal Editor inside Xcode (where you can debug it when it crashes), copy a project from `~/Documents/Unreal Projects` to `~/Documents/UnrealEngine/UnrealEngine`. Rename its encapsulating folder from `UnrealProject1` to `YES` and its project file to `YES.uproject`. This bypasses the `YES/YES.uproject` failure described in "Compile unmodified 'ue5-main'".
