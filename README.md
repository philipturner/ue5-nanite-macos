...
On a local machine, I force-enabled `NaniteAtomicsSupported()` and the build system acted strangely. `XCBBuildService` crashed in the middle of every build, making `UnrealBuildTool` execute in the background. I could not track UnrealBuildTool's progress in Xcode to estimate when it would finish. The second time this happened, I noticed that Clang was still consuming 100% CPU. 
 
Disk space started getting eaten up and I could not find which folder was consuming disk space. `~/Documents/UnrealEngine/UnrealEngine` stayed constant at 199 GB, while <b>Menu Bar >  > About This Mac > Storage</b> showed a gigabyte being consumed every ~10 seconds.
