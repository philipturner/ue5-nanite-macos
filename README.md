...
On a local machine, I force-enabled `NaniteAtomicsSupported()` and the build system acted strangely. `XCBBuildService` crashed in the middle of every build, making `UnrealBuildTool` execute in the background. I could not UnrealBuildTool's progress in Xcode to estimate when it would finish. After this happened two times in a row, I gave up on using Xcode 14 beta.
