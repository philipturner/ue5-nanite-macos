...
To launch the Unreal Editor inside Xcode (where you can debug it when it crashes), copy a project from `~/Documents/Unreal Projects` to `~/Documents/UnrealEngine/UnrealEngine`. This must be like `UnrealProject` described above, but created through <b>GAMES > First Person > Project Defaults > BLUEPRINT</b> instead of <b>C++</b>. Name it `UnrealProject2`.
 
 Rename its encapsulating folder from `UnrealProject2` to `YES` and its project file to `YES.uproject`. This bypasses the `YES/YES.uproject` failure described in "Compile unmodified 'ue5-main'".
