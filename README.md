...
To launch the Unreal Editor inside Xcode (where I can debug it when it crashes), I copied an project from `~/Documents/Unreal Projects`. I pasted it into `~/Documents/UnrealEngine/UnrealEngine` and renamed its encapsulating folder from `UnrealProject1` to `YES`. Then, I renamed the `.uproject` it contains from `UnrealProject1.uproject` to `YES.uproject`. This successfully bypassed the `YES/YES.uproject` failure described in "Compile unmodified 'ue5-main'".
