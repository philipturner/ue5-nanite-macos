...
To debug Unreal Editor when it crashes, it must be launchable from within Xcode. Repeat the process for creating `UnrealProject1`, using the Blueprint template instead of C++. `UnrealProject1` was difficult to work with because I created it with Xcode 14 beta. Switching to Xcode 13 caused a popup that requested to recompile missing modules. Blueprint projects are easier to work with, so I recommend using them from now on.
