...
Follow [this guide](https://docs.unrealengine.com/5.0/en-US/downloading-unreal-engine-source-code), starting with step 3 of "Downloading the Source Code". Make sure to right-click `UE5.xcworkspace` and <b>Open With > Xcode-beta</b>. The instructions below are adapted from [another guide](https://docs.unrealengine.com/5.0/en-US/building-unreal-engine-from-source), which is slightly outdated; no `UE4Editor` or `UE5Editor` scheme exists. Do not run through the latter guide.
...
> Skip the rest of this section, unless you want to save custom modifications to the cloud. Waiting for this `git fetch` is not fun and consumes lots of disk space (~30 GB). It might have been faster/consumed less disk space to omit `--depth 1` from the original `git clone` command.
